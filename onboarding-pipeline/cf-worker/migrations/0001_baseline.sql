-- Migration 0001: Baseline schema (matches production state as of 2026-04-28)
--
-- IMPORTANT: This migration uses CREATE TABLE IF NOT EXISTS (idempotent, safe).
-- It NEVER drops tables. To run on a fresh DB:
--   wrangler d1 execute nexgen-prod --file migrations/0001_baseline.sql
-- To run on prod for re-validation (no-op if tables exist):
--   wrangler d1 execute nexgen-prod --file migrations/0001_baseline.sql
--
-- For destructive fresh-init (test environments only), use:
--   schema-fresh-init.sql

...
例如：「你嘅服務器於 14:32 短暫斷網，已自動恢復」
     「你嘅 bot token 因 Telegram BotFather 嘅安全更新而需要重新驗證」
...
我哋檢查過你嘅服務，技術上一切正常。問題可能係喺 Telegram 客戶端：

請逐項嘗試
────────────────────
1. 確認你冇 block 個 bot：打開與 {bot_username} 嘅對話 → 點擊 bot 名稱 → 確認冇出現「Unblock」按鈕
2. 重啟 Telegram app
3. 確認你嘅網絡連線正常
4. 發送 /start 重新啟動對話
...
由於需要進一步檢查，預計需要 {eta} 小時內完成。期間 bot 嘅服務可能會有短暫中斷。
...
我哋嘅 AI 服務供應商 {provider_name} 目前出現服務中斷，影響所有 NexGen 客戶。

事件詳情
────────────────────
影響時間：{outage_start} 至今
影響範圍：對話功能{additional_impact}
我哋嘅應對：監察緊供應商修復進度，亦同時準備後備方案

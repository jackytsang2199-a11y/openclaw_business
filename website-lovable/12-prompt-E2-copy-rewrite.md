# Prompt E2: Copy & Content Rewrite

> Apply AFTER Prompt E1 (rebrand, color, structure).
> This prompt rewrites all copy, adds new content sections, and updates text across the site.
> It does NOT change colors, layout structure, or remove sections — E1 already handled that.

---

## Voice & Tone

Apply this voice across ALL copy in this prompt:
...
2. **H1** — replace `最強 AI 智能體`:
```
專屬於你的全配版 AI 智能體
```

3. **Diff line** — replace `ChatGPT 做不到的，它全部做到。`:
```
獨家插件生態，ChatGPT 做不到的它全部做到
```
Style: `text-lg md:text-xl text-white/80`

4. **3 checkmarks** — keep as-is
5. **Process line** — keep as-is
6. **CTAs** — keep as-is
...
**Important:** The mockup rendering logic needs to handle AI messages appearing first. Adjust bubble layout so AI messages appear on the left and user messages on the right, regardless of message order.
...
## 4. Plugin Ecosystem + Tech Stack — REWRITE (Index.tsx)
...
Replace the current 6 cards with these 7. Keep same card layout and link to `/technology`:
...
In the Story + Trust + Final CTA section, replace the blockquote text.
...
| CTA button | Keep as-is | Keep as-is |
...
**4 feature cards below the body (in a 4-column grid):**
...
## 10. Technology Page — Copy Fixes (Technology.tsx)
...
### 10C. Add new tech sections
Add these to the existing tech list:
...
| 9 | FAQ device support fix | FAQ.tsx |
| 10 | Technology page copy + new tech items | Technology.tsx |
